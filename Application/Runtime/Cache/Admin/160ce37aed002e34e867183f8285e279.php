...
<body>
	...
	<table border="0" cellspacing="0px" cellpadding="10px" id="table2" >	
		...
		<th>身份证号</th>
		<th>开户时间</th>
			<?php if($_SESSION['admin']['usernature']== 9): ?><th>所属核心</th>
					<th>所属特约</th>
					<th>所属经销</th>
					<th>所属代理</th>
		...
		<td><?php echo ($k+1); ?></td>
		<td>
			<?php if($v['usernature'] == 9): ?>超级管理员
				<?php elseif($v['usernature'] == 1): ?>
				核心用户
				<?php elseif($v['usernature'] == 2): ?>
				特约用户
				<?php elseif($v['usernature'] == 3): ?>
				经销用户
				<?php elseif($v['usernature'] == 4): ?>
				居间用户
				<?php elseif($v['usernature'] == 0): ?>
				普通用户<?php endif; ?>
		</td>
		...
		<td><?php echo ($v['ts']); ?></td>
			...
			<?php if($_SESSION['admin']['usernature']== 9): ?><th><?php echo ($v["hexin"]); ?></th>
				<th><?php echo ($v["teyue"]); ?></th>
				<th><?php echo ($v["jingxiao"]); ?></th>
				<th><?php echo ($v["jujian"]); ?></th>
				<?php elseif($_SESSION['admin']['usernature']== 1): ?>
				<th><?php echo ($v["teyue"]); ?></th>
				<th><?php echo ($v["jingxiao"]); ?></th>
				<th><?php echo ($v["jujian"]); ?></th>
		...
		<td class="color-r del">
			<!--<a href="<?php echo U('upd',array('userid'=>$v['userid']));?>">删除</a>-->
	...
	<div class="pop tan">
		...
		<label for="">公司名称：</label>
			...
			<select name="usernature" class="option">				
				...
				<?php if($_SESSION['admin']['usernature']== 9): ?><option value="1">核心合作商</option>
					...
					<option value="4">居间商</option>
					<?php elseif($_SESSION['admin']['usernature']== 3): ?>
					<option value="4">居间商</option><?php endif; ?>
			</select><br />
...
$('.set').click(function(e){
    ...
    $(".pop").find('form').attr('action',"/index.php/Admin/Index/upuser/userid/"+userid);
	$.ajax({
		url:"/index.php/Admin/Index/userinfo",
		type:"get",
		data:{"userid":userid},
		dataType:"json",
...
// 	$('body').css("background","rgba(22,22,22,.5)");
//
// 	$(".pop").find('form').attr('action',"/index.php/Admin/Index/upuser/userid/"+id);
//
//}
