        ...
        if(IS_POST)
        {
            ...
            if ($tel) {
                ...
                if (preg_match("/^1[34578]{1}\d{9}$/", $is_admin)) {
                    ...
                    if (empty($is_member)) {
                        $this->display('subscription-all');exit;
                    }
                }
                ...
                if ($tel) {
                    if ($starttime != null || $endtime != null) {
                        ...
                        $data = M('userstock')
                            ...
                            ->where($wOne)
                            ->field("user.userid,user.usercode,user.compname,ttproduct.productname,ttproduct.openprice,userstock.num,userstock.summny,userstock.ts")
                            ->select();
            ...
            foreach ($data as $k=>$v)
            {
                if(!in_array($v['userid'],$_SESSION['countid']))
                {
                    unset($data[$k]);
                }
            }
...
//            $hy = $user->query($sql);
        ...
        foreach ($data as $k=>$v)
        {
            $w['userid'] = array("in",$v['upuserid']);
            $w['lev'] = 1;
            $hexin = M('user')->where($w)->getField('compname');
            $ww['userid'] = array('in',$v['upuserid']);
            $ww['lev'] = 2;
            $teyue = M('user')->where($ww)->getField('compname');
            $wwww['userid'] = array('in',$v['upuserid']);
            $wwww['lev'] = 3;
            $jingxiao = M('user')->where($wwww)->getField('compname');
            $wwwww['userid'] = array('in',$v['upuserid']);
            $wwwww['lev'] = 4;
            $jujian = M('user')->where($wwwww)->getField('compname');
            $data[$k]['hexin'] = $hexin;
            $data[$k]['teyue'] = $teyue;
            $data[$k]['jingxiao'] = $jingxiao;
            $data[$k]['jujian'] = $jujian;
    ...
    public function upuser()
    {
        ...
        if (!empty($_POST)) {
            ...
            if ($result) {
                ...
                $this->redirect('Index/client');
            } else {
                $this->success('修改失败', U('client'));
...
//                                                   where find_in_set($upuserid1,user.upuserid) order by subscribe.ts desc";
                        ...
                        $da = M('subscribe')
                            ...
                            ->where($where)
                            ->field("user.userid,subscribe.id,subscribe.num,user.usercode,user.compname,user.freezebalance,ttproduct.productname,subscribe.status,subscribe.ts,subscribe.summny")
